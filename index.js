...
function customObjectFromEntries(entries) {
	if (!Array.isArray(entries)) {
	 ...
	 const massivMap = entries.map(([key, value]) => {
	 ...
	 const objectFromEntries = Object.fromEntries(massivMap);
...
function extendAndFreezeObject(obj1, obj2) {
	...
	const freeze = Object.freeze(assign);
	const isFrozen = Object.isFrozen(freeze)
	if (isFrozen === true) {
		return obj1;
	} else {
		return "Error"
...
function getObjectValuesSum(obj) {
	...
	const values = Object.values(obj);
  	const sum = values.reduce((callback, initialValue) => callback + initialValue, 0);
  	return sum;
